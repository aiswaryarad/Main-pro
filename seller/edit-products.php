...
$sql=mysqli_query($con,"update  product set price='$productprice',Total_quantity='$productquantity',product_desc='$productdescription' where product_id='$pid' ");
...
<div class="controls">
<input type="number" name="productprice" min=1 max=1000000 pattern="\d+((,|\.)\d\d?)*"  placeholder="Enter Product Price" value="<?php echo htmlentities($row['price']);?>" class="span8 tip" required>
</div>
</div>
<div class="control-group">
<label class="control-label" for="basicinput">Toatal Quantity</label>
<div class="controls">
<input type="number" style="height:30px"name="quantity" min=10 max=1000 placeholder="Enter Product quantity" value="<?php echo htmlentities($row['Total_quantity']);?>" class="span8 tip" pattern="\d+((,|\.)\d\d?)*"  required>
</div>
</div>
<div class="control-group">
