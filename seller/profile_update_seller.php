...
$sql=mysqli_query($con,"update users,seller set username='$name',email='$email',contact_no='$contact_no',brandname='$brandname' where users.email = '$email' and users.user_id=seller.user_id");
...
<?php
// $id=intval($_GET['id']);
$query=mysqli_query($con,"SELECT * FROM users,seller WHERE users.email = '$email' and users.user_id=seller.user_id");
...
<div class="controls">
<input  type="text" name="brandname"  placeholder="Enter Brandname" value="<?php echo htmlentities($row['brandname']);?>" class="span8 tip" required>
</div>
</div>
<div class="control-group">
<label class="control-label" for="basicinput">Certificate</label>
<div class="controls">
<img src="../certificate_upload/<?php echo htmlentities($row['certificate']);?>" width="200" height="100"> <a href="update-image.php?id=<?php echo $row['seller_id'];?>"></a>
<td><button class="btn"> <a href="../certificate_upload/reg certif.jpg">view</i></button></td>
</div>
</div>
<?php } ?>
<div class="control-group">
