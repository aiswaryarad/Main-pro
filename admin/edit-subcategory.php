...
$select = "SELECT * FROM subcategory WHERE subcategory ='$subcategory'";
    $result=$con->query($select);
    if($result->num_rows>0)
        {
            $_SESSION['msg']="category already Created !!";     
        }
        else{

$sql=mysqli_query($con,"update subcategory set category_id= $category,subcategory='$subcategory' where id='$id'");
...
<?php } ?>
<br />
<form class="form-horizontal row-fluid" name="insertsubcategory" method="post" enctype="multipart/form-data">
<?php
$id=intval($_GET['id']);
$query=mysqli_query($con,"select subcategory.*,category.category_name as catname,category.category_id as cid  from subcategory join category on category.category_id=subcategory.category_id  where subcategory.id='$id'");
...
<div class="controls">
<input type="text" name="subcategory" pattern="^([A-Za-z]+ )+[A-Za-z]+$|^[A-Za-z]+$" placeholder="Enter subcategory Name" value="<?php echo htmlentities($row['subcategory']);?>" class="span8 tip" required>
</div>
</div>
<?php } ?>
<div class="control-group">
